...
from langchain_core.stores import BaseStore          # 新版导入路径

class SQLiteDocStore(BaseStore):
    """基于 SQLite 的文档存储，用于 MultiVectorRetriever 的 byte_store"""
    ...
    def mget(self, keys: List[str]) -> List[Optional[object]]:
        ...
        try:
            ...
            sql = f"SELECT doc_id, data FROM {self.table_name} WHERE doc_id IN ({placeholders})"
            rows = conn.execute(sql, keys).fetchall()
            # 构建 key -> data 的映射
            data_map = {row[0]: row[1] for row in rows}
            # 按 keys 的顺序返回，若不存在则返回 None
            return [pickle.loads(data_map[k]) if k in data_map else None for k in keys]
        finally:
            conn.close()

    def mset(self, key_value_pairs: List[Tuple[str, object]]) -> None:
        """批量插入或更新文档对象"""
        conn = self._get_conn()
        try:
            rows = [(k, pickle.dumps(v)) for k, v in key_value_pairs]
            conn.executemany(
                f"INSERT OR REPLACE INTO {self.table_name} (doc_id, data) VALUES (?, ?)",
                rows
            )
            conn.commit()
        finally:
    ...
    def yield_keys(self, prefix: Optional[str] = None) -> Iterator[str]:
        """遍历所有 doc_id（可选实现，用于维护）"""
        conn = self._get_conn()
        try:
            if prefix:
                sql = f"SELECT doc_id FROM {self.table_name} WHERE doc_id LIKE ?"
...
doc_store = SQLiteDocStore(db_path="documents.db")   # 文件路径可按需修改
...
'''
(venv-rag-langchain) thbytwo@thbytwopower:~/testGit/rag-in-action$ /home/thbytwo/miniforge3/envs/venv-rag-langchain/bin/python "/home/thbytwo/testGit/rag-in-action/06-索引优化-Indexing/03-构建多表示的索引/02-用MultiVectorRetriever构建多表示索引-Milvus-sqlite-ollama.py"
/home/thbytwo/testGit/rag-in-action/06-索引优化-Indexing/03-构建多表示的索引/02-用MultiVectorRetriever构建多表示索引-Milvus-sqlite-ollama.py:132: LangChainDeprecationWarning: The class `OllamaEmbeddings` was deprecated in LangChain 0.3.1 and will be removed in 1.0.0. An updated version of the class exists in the :class:`~langchain-ollama package and should be used instead. To use it run `pip install -U :class:`~langchain-ollama` and import as `from :class:`~langchain_ollama import OllamaEmbeddings``.
  embed_model_ollama = OllamaEmbeddings(
/home/thbytwo/testGit/rag-in-action/06-索引优化-Indexing/03-构建多表示的索引/02-用MultiVectorRetriever构建多表示索引-Milvus-sqlite-ollama.py:138: LangChainDeprecationWarning: The class `Ollama` was deprecated in LangChain 0.3.1 and will be removed in 1.0.0. An updated version of the class exists in the :class:`~langchain-ollama package and should be used instead. To use it run `pip install -U :class:`~langchain-ollama` and import as `from :class:`~langchain_ollama import OllamaLLM``.
  llm_ollama = Ollama(
...
开始检索...
/home/thbytwo/testGit/rag-in-action/06-索引优化-Indexing/03-构建多表示的索引/02-用MultiVectorRetriever构建多表示索引-Milvus-sqlite-ollama.py:230: LangChainDeprecationWarning: The method `BaseRetriever.get_relevant_documents` was deprecated in langchain-core 0.1.46 and will be removed in 1.0. Use :meth:`~invoke` instead.
  retrieved_docs = retriever.get_relevant_documents(query, n_results=1)
