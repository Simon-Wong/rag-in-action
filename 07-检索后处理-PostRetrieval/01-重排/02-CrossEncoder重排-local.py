...
print("加载预训练模型...")
local_model_path = "/mnt/d/modelscope_stuff/cross-encoder/ms-marco-MiniLM-L12-v2" #cross-encoder/ms-marco-MiniLM-L-12-v2 是MS MARCO数据集上训练的小型模型
...
print("- 计算密集：每个查询-文档对都需要单独编码")
print("- 最佳实践：用于对初检索结果进行精细重排")



'''
(venv-rag-langchain) thbytwo@thbytwopower:~/testGit/rag-in-action$ /home/thbytwo/miniforge3/envs/venv-rag-langchain/bin/python /home/thbytwo/testGit/rag-in-action/07-检索后处理-PostRetrieval/01-重排/02-CrossEncoder重排-local.py
初始化CrossEncoder重排模型...
加载预训练模型...
使用模型: /mnt/d/modelscope_stuff/cross-encoder/ms-marco-MiniLM-L12-v2
  - 该模型在MS MARCO段落检索任务上进行了微调
  - 专门优化用于计算查询-段落相关性分数
  - 平衡了模型大小和性能，适合生产环境使用
模型加载完成
...
CrossEncoder重排总结:
- 深度语义理解：捕捉查询与文档间的细粒度交互
- 精确相关性建模：端到端训练获得准确的相关性分数
- 上下文感知：考虑词汇的位置信息和上下文关系
- 计算密集：每个查询-文档对都需要单独编码
- 最佳实践：用于对初检索结果进行精细重排
